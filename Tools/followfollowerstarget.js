...
const CommentAndLike = async function(session, accountId, text){
  ...
  if (result.length > 0) {
    ...
    const printFollow = Follow ? chalk`{green SUKSES}` : chalk`{red FOLLOW}`;
    const printComment = Comment ? chalk`{green SUKSES}` : chalk`{red COMMENT}`;
    const printLike = Like ? chalk`{green SUKSES}` : chalk`{red LIKE}`;
