...
const CommentLikeDM = async function(session, accountId, text){
  ...
  if (result.length > 0) {
	...
	const printDM = DM ? chalk`{green SUKSES}` : chalk`{red GAGAL}`;
    const printLike = Like ? chalk`{green SUKSES}` : chalk`{red GAGAL}`;
  return chalk`[Follow: ${printFollow}] [Direct Message: ${printDM}] [Like: ${printLike}] [Comment: ${printComment} ({cyan ${text}})]`;
  }
